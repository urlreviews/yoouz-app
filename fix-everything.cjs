...
// 1. Remove my broken appended isVideoCard block at the bottom
const brokenBlockRegex = /const isVideoCard = type === 'video';\s*if \(isVideoCard\) \{[\s\S]*?\}\s*if \(isPlaceCard\)/;
code = code.replace(brokenBlockRegex, 'if (isPlaceCard)');

// 2. Add back the `const isVideoCard` where it belongs (at the top where I deleted it originally).
// Wait, I can just replace the `if (isVideoCard) { ... } else if (isPlaceCard)` block.
const origRegex = /if \(isVideoCard\) \{[\s\S]*?\} else if \(isPlaceCard\)/;

const interceptCode = `const isVideoCard = type === 'video';
    if (isVideoCard) {
...
code = code.replace(origRegex, interceptCode);
