...
async function migrateAllPlaces() {
  ...
  for (const [id, info] of Object.entries(ENRICHED_PLACES)) {
    ...
    await bDb.execute({
      sql: `UPDATE places SET name = ?, address = ?, city = ?, country = ?, latitude = ?, longitude = ?, category = ?, data = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?`,
      args: [info.name, info.address, fullCity, info.country, info.lat, info.lng, info.category, JSON.stringify(parsed), id]
    });
